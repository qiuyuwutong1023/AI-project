...
This streamlined repository focuses on Bin Packing Problem (BPP) only, with both black-box and white-box settings.
...
This will install all optional dependencies for this streamlined setup (see the [`pyproject.toml`](pyproject.toml) for details).
...
No extra dependency group is required for `bpp_online`.
...
# e.g., for bpp_online
python main.py \
    problem=bpp_online \  # problem name
...
- Bin Packing Problem (BPP): `bpp_online`
...
- The LLM-generated heuristic is written into `f"./problems/YOUR_PROBLEM/gpt.py"`, then imported by `./problems/YOUR_PROBLEM/eval.py`, which is called by `reevo._run_code` during ReEvo.
- In training mode, `./problems/YOUR_PROBLEM/eval.py` should print the meta-objective value as the last line of stdout, which is parsed by `reevo.evaluate_population` for heuristic evaluation.
