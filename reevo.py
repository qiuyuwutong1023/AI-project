...
class ReEvo:
    ...
    def _extract_thoughts_and_codes(self, response: str) -> tuple[str, Optional[str]]:
        """
        ...
        # Primary parsing path for strict format.
        match = re.search(
            r"(?is)^\s*Thoughts\s*:\s*(.*?)\s*\n\s*Codes\s*:\s*(.*)$",
            content,
        )
        if match is not None:
            thoughts = match.group(1).strip()
            codes_part = match.group(2).strip()
        else:
            # Fallback for slightly off-format outputs.
            split_match = re.search(r"(?i)\bCodes\s*:", content)
            if split_match is not None:
                thoughts_part = content[:split_match.start()]
                thoughts = re.sub(r"(?is)^\s*Thoughts\s*:\s*", "", thoughts_part).strip()
                codes_part = content[split_match.end():].strip()

        code = extract_code_from_generator(codes_part)
        if code is None:
    ...
    def gen_short_term_reflection_prompt(self, ind1: dict, ind2: dict) -> tuple[list[dict], str, str, str, str]:
        """
        ...
        worse_thoughts = worse_ind.get("thoughts", "")
        better_thoughts = better_ind.get("thoughts", "")
        ...
        return message, worse_code, better_code, worse_thoughts, better_thoughts


    def short_term_reflection(self, population: list[dict]) -> tuple[list[list[dict]], list[str], list[str], list[str], list[str]]:
        """
        Short-term reflection before crossovering two individuals.
        """
        messages_lst = []
        ...
        for i in range(0, len(population), 2):
            ...
            messages, worse_code, better_code, worse_thoughts, better_thoughts = self.gen_short_term_reflection_prompt(parent_1, parent_2)
            messages_lst.append(messages)
            worse_code_lst.append(worse_code)
            better_code_lst.append(better_code)
            worse_thoughts_lst.append(worse_thoughts)
            better_thoughts_lst.append(better_thoughts)
        
        # Asynchronously generate responses
        response_lst = self.short_reflector_llm.multi_chat_completion(messages_lst)
        return response_lst, worse_code_lst, better_code_lst, worse_thoughts_lst, better_thoughts_lst
    ...
    def crossover(self, short_term_reflection_tuple: tuple[list[list[dict]], list[str], list[str], list[str], list[str]]) -> list[dict]:
        reflection_content_lst, worse_code_lst, better_code_lst, worse_thoughts_lst, better_thoughts_lst = short_term_reflection_tuple
        messages_lst = []
        for reflection, worse_code, better_code, worse_thoughts, better_thoughts in zip(
            reflection_content_lst,
            worse_code_lst,
            better_code_lst,
            worse_thoughts_lst,
            better_thoughts_lst,
            ...
            user = self.crossover_prompt.format(
                ...
                worse_code = worse_code,
                better_code = better_code,
                worse_thoughts = worse_thoughts,
                better_thoughts = better_thoughts,
